...
1. Orai의 경계를 지킨다. 큐와 receipt는 AMQ, 대화는 provider, 역할 조직과 업무 규칙은 소비 프로젝트가 소유한다. 이 책임을 Orai에 다시 구현하지 않는다.
2. 플러그인 프레임워크, 자체 범용 MCP 서버, GUI, 클라우드 스케줄러를 먼저 만들지 않는다. CLI와 진단을 먼저 완성한다.
3. 기존 계약(정확한 재개, nonce 기반 캡처, 알림은 ID만, 소비하지 않는 notifier, 대화형 stdin 즉시 거부, 하위 도구 종료 코드 보존)을 바꿀 때는 먼저 계약 테스트로 동등성을 보인다.
4. 실패를 성공으로 보고하지 않는다. 빈 검색, 도구 실패, 샌드박스 접근 거부를 대상이 없다는 근거나 서버 다운의 근거로 삼지 않는다.
5. 테스트는 실제 역할 세션, 실제 프로젝트 큐, 사용자의 전역 설정, 다른 프로젝트의 QMD 서버(`8181` 포함)를 건드리지 않는다.
...
| 작업 | 명령 |
|---|---|
| 도구 설치 | `mise install` (Python·uv pin, `mise.lock`) |
| 가상환경 | `mise run setup` (`uv sync --locked`) |
| 정적 검사·포맷 | `mise run lint`, `mise run fmt` |
| 테스트 | `mise run test` (단위·격리 통합), `mise run test:package` (wheel 설치 검증) |
| CI와 같은 전체 검사 | `mise run check` |
| 이 checkout 진단 | `mise run doctor` |

Python 의존성은 `pyproject.toml`과 `uv.lock`에서만 관리한다. 런타임 코드는 표준 라이브러리만 쓴다. 최소 지원 버전은 Python 3.11이다.

템플릿(`src/orai/templates/`)을 바꾸면 `mise run test:package`로 설치본이 같은 템플릿을 배포하는지 확인한다. 그다음 `orai setup`으로 이 저장소의 생성 파일도 갱신한다(`test_this_repository_is_setup_current`가 누락을 잡는다).
...
| 정확한 문자열·파일 | `rg -n '패턴' src tests docs`, `rg --files` |
...
| `docs/architecture.md` | 책임·경계, 식별·상태·데이터 흐름, 진단 모델 |
...
| `docs/release.md` | 배포 단계, 버전 정책, 릴리스 점검 |
